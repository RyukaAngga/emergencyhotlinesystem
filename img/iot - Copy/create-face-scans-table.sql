-- ========================================
-- TABLE: face_scans
-- Menyimpan metadata setiap scan/foto wajah
-- ========================================
...
-- Policy: Allow read untuk semua
CREATE POLICY "Allow read face_scans" ON face_scans
    FOR SELECT
    USING (true);

-- Policy: Allow insert untuk semua (untuk auto scan)
CREATE POLICY "Allow insert face_scans" ON face_scans
    FOR INSERT
    WITH CHECK (true);

-- Policy: Allow update untuk semua
CREATE POLICY "Allow update face_scans" ON face_scans
    FOR UPDATE
    USING (true);

-- Policy: Allow delete untuk semua (opsional, bisa disesuaikan)
